...
function Tauvec= sidanet_tunerange(Xdata,Y,ngrid,standardize,weight, eta, edges,vweight,withCov)
...
%eta balances smoothness and sparsity 
%edges gives edge information for variable-variable connections
%vweight is the edge weight
%withCov is True or False for whether covariates are included or not
...
%obtain  nonsparse input
    ...
    for loc=1:6
        Tau=myperx(:,loc);
        myalpha=sidanet_inner(Xdata,Y, sqrtminv,myalphaoldmat,tildealphamat, tildelambda, Tau,weight, eta, edges,vweight,withCov,myL);
